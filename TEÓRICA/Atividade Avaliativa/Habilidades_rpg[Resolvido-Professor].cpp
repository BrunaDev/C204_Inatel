...
// Funções
//raiz = nó principal  | *& passa uma referencia para essa raiz (passagem de parametros por referencia)
void inserir_habilidade(Node *& raiz, Habilidade h){
	if(raiz == NULL){
		raiz = new Node;
		//colocando o h (habilidade) dentro do nó
		(*raiz).info = h;
		//para garantir que não aponta para alguma outra informação
		(*raiz).esquerda = NULL;
		(*raiz).direita = NULL;
		//por ser um ponteito e uma struct, podemos substituir o (*) por uma ->
		// raiz->info = h;
		// raiz->esquerda = NULL;
		// raiz->direita = NULL;
	} else{
		if(h.codigo < raiz->info.codigo){
			inserir_habilidade(raiz->esquerda, h);
		} else{
			inserir_habilidade(raiz->direita, h);
		}
	}
}
...
Habilidade * buscar_habilidade(Node * raiz, int codigo_habilidade){
	...
	if(codigo_habilidade < raiz->info.codigo){
		return buscar_habilidade(raiz->esquerda, codigo_habilidade);
	} else {
		return buscar_habilidade(raiz->direita, codigo_habilidade);
	}
}

/*
	A função gerar_desafio cria um desafio de um tipo aleatório e com um valor de dificuldade de 0 a 30.
*/
...
int main(){
	...
	do{	
		...
		if(op == 1){
			...
			Habilidade h;
			cout << "Informe o código da habilidade: " << endl;
			cin >> h.codigo;
			cout << "Informe a descrição da habilidade: " << endl;
			cin >> h.descricao;
			cout << "Informe o atributo da habilidade (1:Mental, 2: Físico, 3: Espírito): " << endl;
			cin >> h.atributo_base;
			h.atributo_base -= 1;
			...
			// Buscar habilidade na estrutura de dados | * pois o retorno é um endereço de memória
			Habilidade * resultado = NULL;
			do{
				cout<<"Escolha a sua habilidade: ";			
				cin>>habilidade;
				
				resultado = buscar_habilidade(raiz, habilidade);
